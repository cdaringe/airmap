...
const DataExplorerFlyout = dynamic(
  () => import("../DataExplorerFlyout").then((m) => ({ default: m.DataExplorerFlyout })),
  { ssr: false }
);
...
const getTigerXtStats = (data: TigerXtOnlyEntry[], dateOffsetMinutes: number) => {
  if (!data || data.length === 0) return { count: 0 }

  const timestamps = data
    .map(item => {
      const date = new Date(item.date)
      if (dateOffsetMinutes !== 0) date.setMinutes(date.getMinutes() + dateOffsetMinutes)
      return date
    })
    .filter(d => !isNaN(d.getTime()))
    .sort((a, b) => a.getTime() - b.getTime())

  const isobutyleneValues = data.map(item => item.isobutylene).filter(v => typeof v === 'number')

  return {
    count: data.length,
    minDate: timestamps[0] || undefined,
    maxDate: timestamps[timestamps.length - 1] || undefined,
    duration: timestamps[0] && timestamps[timestamps.length - 1]
      ? timestamps[timestamps.length - 1].getTime() - timestamps[0].getTime() : undefined,
    metrics: isobutyleneValues.length > 0 ? [{
      key: 'isobutylene', label: 'Isobutylene',
      min: Math.min(...isobutyleneValues), max: Math.max(...isobutyleneValues),
      avg: isobutyleneValues.reduce((a, b) => a + b, 0) / isobutyleneValues.length, unit: 'µg/m³'
    }] : undefined
  }
}
...
const applyOffsetToTigerXtData = (data: TigerXtOnlyEntry[], offsetMinutes: number): TigerXtOnlyEntry[] => {
...
const applyOffsetToStravaData = (data: StravaEntry[], offsetMinutes: number): StravaEntry[] => {
  return data.map(item => ({
    ...item,
    date: new Date(new Date(item.date).getTime() + offsetMinutes * 60 * 1000)
  }))
...
export const TigerXtDataSource: React.FC<{
  ...
  }>({ isOpen: false, data: null, title: '', columns: [], getStats: () => ({ count: 0 }) });

  const [combinedData, setCombinedData] = React.useState<any[] | null>(null);
  const [combinedError, setCombinedError] = React.useState<string>('');
    ...
    function submitOnReady() {
      if (!err && tigerXtData && stravaData) {
        ...
        try {
          ...
          setCombinedData(combined.features || []);
          if (!combined.features || combined.features.length === 0) {
            setCombinedError('No data points found after combining Tiger XT and GPS data. Check timestamps alignment.');
          } else {
            setCombinedError('');
          }
        } catch (error) {
          setCombinedError(`Error combining data: ${String(error)}`);
          ...
          Array.from(filelist).map(async (file) => {
            ...
            switch (ext) {
              ...
              case "gpx": {
                ...
                console.debug('setting strava data', records);
                ...
                onClick={() => {
                  setExplorerData({
                    ...
                    onApplyOffset: (offsetMinutes: number) => {
                      if (tigerXtData) {
                        setTigerXtData(applyOffsetToTigerXtData(tigerXtData, offsetMinutes))
                      }
                    }
                  });
                }}
              ...
              <Button
                ...
                onClick={() => {
                  setExplorerData({
                    ...
                    onApplyOffset: (offsetMinutes: number) => {
                      if (stravaData) {
                        console.debug('setting strava data', stravaData);
                        setStravaData(applyOffsetToStravaData(stravaData, offsetMinutes))
                      }
                    }
                  });
                }}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-600 text-sm mt-2">
                Try inspecting your data above to check timestamp alignment and data quality.
              </p>
            </div>
          </div>
        </div>
